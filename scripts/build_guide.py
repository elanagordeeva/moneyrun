...
while i<n:
    ...
    if ln.replace('\xa0',' ').rstrip().endswith('активируется следующими способами:'):
        ...
        i=j; continue
    # условия RQ («Если результативность …») -> буллеты
    if ln.startswith('Если результативность'):
        items=[]; j=i
        while j<n:
            s=lines[j].strip()
            if not s: j+=1; continue
            if not s.startswith('Если результативность'): break
            items.append(s); j+=1
        lis=''.join(f'<li>{style(it.rstrip(" ;"))}</li>' for it in items)
        body.append('<ul class="g-bullets">'+lis+'</ul>')
        i=j; continue
    ...
    if ln.replace('\xa0',' ').strip()=='Текущий Грейд ~ текущий RL':
        body.append('<figure class="g-rl-vis"><img src="art/Graph.svg" alt="Зависимость RQ от соотношения RL и Грейда" loading="lazy"></figure>')
        skipped=0; j=i+1
        while j<n and skipped<2:
            s=lines[j].strip(); j+=1
            if s: skipped+=1
        i=j; continue
    # примеры с Иваном/Марией -> в рамку
    if re.match(r'^Атлет (Иван|Мария) с', ln):
        body.append('<div class="g-example"><p>'+style(ln.rstrip(" ;"))+'</p></div>')
        i+=1; continue
...
CSS = """
    ...
    *{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth;font-size:16px}
    ...
    .tg-pill{display:inline-flex;align-items:center;gap:.4rem;background:#fff;color:var(--ink);padding:.5rem .85rem;border-radius:999px;text-decoration:none;font-size:.88rem;font-weight:500;border:1px solid var(--border);transition:all .25s}
    .tg-pill:hover{border-color:var(--ink);transform:translateY(-1px)}
    .tg-pill svg{width:16px;height:16px;fill:currentColor}
    ...
    .g-hero-sub{color:#2c466f;font-size:clamp(1rem,1.4vw,1.12rem);line-height:1.6;max-width:32rem;margin:0}
    ...
    .legal h2{font-family:var(--fd);font-weight:800;font-size:clamp(1.7rem,3vw,2.4rem);letter-spacing:-.025em;line-height:1.1;margin:2.8rem 0 1rem;padding-top:.4rem;color:var(--ink)}
    .legal h3{font-family:var(--fd);font-weight:800;font-size:clamp(1.25rem,1.9vw,1.55rem);letter-spacing:-.02em;line-height:1.15;margin:1.8rem 0 .5rem;color:var(--ink)}
    .g-h{position:relative;scroll-margin-top:84px}
    /* салатовые заголовки — фирменная подсветка (тёмный текст на лайме) */
    .g-h-mark{display:inline;background:none;color:inherit;padding:0;border-radius:0;box-shadow:none}
    .legal h3.g-h3 .g-h-mark{border-radius:0;padding:0}
    /* акцентный наклон оставляем только для hero h1 и финального заголовка */
    .g-hero h1,.g-final-title{font-style:italic;font-variation-settings:'slnt' -10}
    ...
    .g-final-mark{display:inline;color:inherit;padding:0}
    .g-final-mark::before{display:none}
    ...
    @media(max-width:760px){
      .g-rl-vis{padding:.75rem .5rem;overflow-x:auto;-webkit-overflow-scrolling:touch}
      .g-rl-vis img{width:auto;max-width:none;height:240px;margin:0}
    }
    @media(max-width:420px){
      .g-rl-vis img{height:210px}
    }
    ...
    .g-example p{margin:0;color:var(--ink2);line-height:1.65;font-size:.95rem}
    ...
    .g-acc-item summary{list-style:none;cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:.7rem;padding:.6rem .9rem;font-family:var(--fd);font-weight:700;font-size:.76rem;color:var(--ink)}
    .g-acc-item summary::-webkit-details-marker{display:none}
    .g-acc-item summary::after{content:'+';font-family:var(--fd);font-weight:700;font-size:1.1rem;line-height:1;color:var(--muted2);transition:color .2s}
    ...
    table.g-tbl,table.g-matrix{border-collapse:collapse;width:100%;font-size:.74rem}
    table.g-tbl th,table.g-tbl td,table.g-matrix th,table.g-matrix td{padding:.26rem .45rem;text-align:left;border-bottom:1px solid var(--border);white-space:nowrap}
    ...
    .g-matrix .g-grp .g-c-blue{background:#74B3FF;color:#fff}
    ...
    .g-matrix tbody .g-c-green{background:rgba(156,209,97,.14)}
    .g-matrix tbody .g-c-blue{background:rgba(116,179,255,.10)}
    /* компактная Матрица: плотнее по обеим осям */
    table.g-matrix{font-size:.74rem}
    table.g-matrix th,table.g-matrix td{padding:.26rem .45rem}
...
NAV = """<nav id="navbar"><div class="nav-inner">
  <a href="index.html" class="nav-logo" aria-label="Moneyrun"><img class="logo-mark" src="art/logo-hor-black.svg" alt="Moneyrun" height="28"></a>
  <div class="nav-right" style="display:flex;align-items:center;gap:.65rem">
    ...
    <a href="https://t.me/moneyrun" target="_blank" rel="noopener" class="tg-pill"><svg viewBox="0 0 24 24"><path d="M23.91 3.79L20.3 20.84c-.25 1.21-.98 1.5-2 .94l-5.5-4.07-2.66 2.57c-.3.3-.55.56-1.1.56-.72 0-.6-.27-.84-.95L6.3 13.7l-5.45-1.7c-1.18-.35-1.19-1.16.26-1.75l21.26-8.2c.97-.43 1.9.24 1.53 1.73z"/></svg>Telegram</a>
...
<main class="legal"><div class="legal-wrap">
    ...
    <header class="g-hero">
      ...
      <div class="g-hero-art" aria-hidden="true">
        <img src="art/guide-hero.svg?v=2" alt="Тренировочная нагрузка" loading="eager">
      </div>
    </header>
    {content}
</div></main>
